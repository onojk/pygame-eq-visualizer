...
while running:
    ...
    for dot in dots:
        ...
        if STATE == "coalescing":
            if dist > 50:
                all_close = False
            norm_dx = dx / max(1e-4, dist)
            norm_dy = dy / max(1e-4, dist)

            gravity_strength = 2000  # tweakable
            force = gravity_strength / (dist + 4)  # inverse pull
            force = min(force, 20)  # optional cap

            dot['pos'][0] += norm_dx * force * dot['speed_mult']
            dot['pos'][1] += norm_dy * force * dot['speed_mult']
